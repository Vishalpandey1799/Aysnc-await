  ...
  return (
    <div className="space-y-8 pb-12 max-w-7xl mx-auto">
          ...
          <div className="p-4 md:p-6">
            <div className="aspect-video w-full rounded-xl border border-blue-900 bg-neutral-800/60 relative overflow-hidden ">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 py-2 gap-5">
                ...
                <h3 className="text-2xl md:text-3xl font-extrabold">Visual Narrator</h3>
                <p className="mt-2 max-w-xl text-neutral-300 text-sm md:text-base">
                  Click "Start Narration" to begin. The AI will identify objects in your camera feed and create engaging stories in real-time.
                </p>
               <button
                  className="flex items-center justify-center gap-2 rounded-xl px-8 py-4 text-xl font-semibold text-white bg-gradient-to-r from-blue-600 via-purple-500 to-purple-400 hover:from-blue-700 hover:to-pink-600 cursor-pointer hover:shadow-xl hover:shadow-blue-900/50 hover:-translate-y-1 transition-all duration-300 ease-in-out"
                >
              ...
              {/* Soft frame */}
              <div className="absolute inset-0 rounded-xl ring-1 ring-black/10 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.02)]" />
            </div>
          </div>
        </motion.div>
            ...
            <div className="mt-4">
              <div className="relative">
                <div className="ml-4 inline-block rounded-2xl bg-blue-600 text-white px-4 py-3 shadow-lg">
                  <span className="font-semibold">Chronicle AI:</span> Hello! I'm ready to narrate your visual world. Click the start button when you're ready to begin!
                </div>
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {narrativeStyles.map((s) => (
                  <span key={s.label} className={`${styles.chip} ${s.featured ? "ring-1 ring-blue-400/50" : ""}`}>
                    {s.featured && <Star className="h-3.5 w-3.5 text-yellow-400 fill-yellow-400" />}
                    {s.label}
                  </span>
                ))}
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatusCard icon={<Activity className="text-blue-400" />} label="Status" value="Ready" />
        <StatusCard icon={<MessageSquare className="text-purple-300" />} label="Objects Detected" value="0" />
        <StatusCard icon={<Bot className="text-pink-300" />} label="Voice Output" value={muted ? "Muted" : "Enabled"} />
        <StatusCard icon={<Gauge className="text-green-300" />} label="Processing" value="Idle" />
      </div>
        ...
        <div className={`lg:col-span-2 ${styles.card}`}>
          <div className="p-4 md:p-6">
            ...
            <ul className="mt-4 space-y-3">
              {history.map((h) => (
                <li key={h.id} className="flex items-start gap-3 rounded-xl border border-neutral-800 bg-neutral-900/40 p-3">
